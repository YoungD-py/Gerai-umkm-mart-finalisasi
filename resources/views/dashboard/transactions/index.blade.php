                            ...
                            <tr>
                                ...
                                <th>
                                    ...
                                    <button type="button" class="btn btn-sm btn-light ms-2 sort-toggle"
                                        ...
                                        data-sort-order="{{ request('sort_status', 'none') }}"
                                        title="Urutkan berdasarkan status">
                                        <i class="bi {{ request('sort_status') == 'failed_first' ? 'bi-sort-up text-warning' : (request('sort_status') == 'success_first' ? 'bi-sort-down text-success' : 'bi-arrow-down-up') }}"></i>
                                    </button>
                                </th>
                                <th>Total</th>
                                <th>Bayar</th>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        directDownloadLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // Buat form tersembunyi untuk POST request
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = '/dashboard/cashiers/nota';
                ...
                // Tambahkan no_nota
                const noNotaInput = document.createElement('input');
                noNotaInput.type = 'hidden';
                noNotaInput.name = 'no_nota';
                noNotaInput.value = this.getAttribute('data-no-nota');
                form.appendChild(noNotaInput);
        ...
        // --- Handle download nota untuk baris lainnya (form biasa) ---
        const downloadNotaForms = document.querySelectorAll('.download-nota-form');
        downloadNotaForms.forEach(form => {
